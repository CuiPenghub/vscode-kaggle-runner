...
export async function initProject(root: string, context: vscode.ExtensionContext) {
  let savedUsername = '';
  try {
  ...
  const title = await vscode.window.showInputBox({
    ...
    value: 'My Awesome Kernel',
  });
  if (!title) return;

  const username = await vscode.window.showInputBox({
    prompt: 'Your Kaggle username (for kernel slug)',
    value: savedUsername,
  });
  if (!username) return;
  ...
  const codeFile =
    (await vscode.window.showQuickPick(['notebook.ipynb', 'main.py'], {
      placeHolder: 'Primary code file',
    })) || 'notebook.ipynb';

  const kernelId = `${username}/${title
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/(^-|-$)/g, '')}`;
...
outputs:\n  download_to: .kaggle-outputs/\n`;
  ...
  if (!(await exists(path.join(root, codeFile)))) {
    if (codeFile.endsWith('.py')) {
      ...
      await writeFile(
        path.join(root, codeFile),
        JSON.stringify({ cells: [], metadata: {}, nbformat: 4, nbformat_minor: 5 }, null, 2)
      );
    }
  }
